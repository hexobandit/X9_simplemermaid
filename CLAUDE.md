...
- `try-in-editor.js` - **Auto "Try in Editor" links** (shared, self-contained). Loaded on `learn-mermaid.html` and all 12 blog articles. Scans `.code-block` elements, detects real Mermaid content by first keyword (skips bash/YAML/JS/markdown examples), and appends a "Try in Editor →" button deep-linking to the editor via `#diagram=<lzstring>` (loads LZ-String from CDN on demand). Skips blocks that already have a hand-authored `#diagram=`/`?example=` link within the next 2 sibling elements. Fires GA event `try_in_editor`.
- `promo.js` - **Cross-promo toast widget** (shared, self-contained: injects its own CSS + markup). Loaded via `<script src="promo.js" defer>` (`../promo.js` in subdirs) on all content pages except `privacy.html`/`terms.html`/`tests.html`. Promotes sister sites (defensemodeling.com, military-aircraft-tracker.com, legenderby.io) in rotation via a small bottom-right toast. Politeness rules baked in: appears only after 30s of *visible* dwell time (60s on the editor via `data-delay="60000"`), max once per session and once per 12h, dismiss snoozes 12h, click-through snoozes 48h. State in localStorage keys `simplemermaid-promo-*`. Fires GA events (`promo_impression`/`promo_click`/`promo_dismiss` under category `cross_promo`). To add/edit promoted sites, edit the `SITES` array at the top of the file.
